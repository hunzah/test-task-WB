...
validateInput(phoneInput, phoneError, /^\+?[0-9]+$/, 'Веедите корректный номер');
...
validateInput(mailInput, mailError, /^[^\s@]+@[^\s@]+\.[^\s@]+$/, 'Веедите корректный email');

const innInput = document.querySelector('.receiver-INN-input');
const innError = document.querySelector('.receiver-input-text');
validateInput(innInput, innError, /^[^\s@]+@[^\s@]+\.[^\s@]+$/, 'Веедите корректный ИНН');
